...
import { str_escape } from './string.js'
...
function resolve_shebang(conf, lang, suffix)
{
	if (conf.shebang) {
		...
		else if (suffix && HAS_PROP(conf.shebang, suffix))
...
function resolve_license(conf, lang, suffix)
{
	...
	else if (suffix && HAS_PROP(conf.license, suffix))
...
function resolve_copyright(conf, lang, suffix)
{
	...
	else if (suffix && HAS_PROP(conf.copyright, suffix))
...
export function format_resolve_copyright(conf, lang, suffix)
{
	const [ copyright, key ] = resolve_copyright(conf, lang, suffix)
...
export function format_use_copyright(conf, lang, suffix)
{
	...
	if (!conf.copyright_rule)
		...
		key = lang
	else if (suffix && HAS_PROP(conf.copyright_rule, suffix))
		key = suffix
	else
		return 0
...
export function format_to_regex(fmt, repl)
{
	const dirty = fmt.split(/REPLACE_STR_RE/)
	const clean = dirty.map(str_escape)
